...
| Risk Manager | Enforces position sizing, drawdown limits, volatility regime filters | **The Discipline** — protects |
...
### Intraday Checkpoint Loop (6× per Trading Day)
...
> *Checkpoints run at 9:30 AM (CP1), 10:30 AM (Mon-1), 11:30 AM (Mon-2), 12:30 PM (Mon-3), 1:30 PM (Mon-4), and 2:30 PM (Mon-5) IST. The evening batch generates the overnight baseline plan; checkpoints replace it with live-price decisions during market hours.*
> **Checkpoint 1 (9:30 AM) runs the full pipeline above.** Mon-1 through Mon-5 (10:30 AM–2:30 PM) run a lightweight subset: price fetch only → stop-loss checks → drawdown checks → volatility regime check. No inference, no re-ranking, no new buy proposals at Mon-1 through Mon-5.
...
| D-006  | Collect Sensex index data daily for both relative strength benchmark (feature engineering) and volatility regime detection (G-007/G-008); volatility regime is computed as the **Sensex 20-day annualized realized volatility** from Sensex daily closes; source: same as D-007 (AlphaVantage for bootstrap, BSE Bhavcopy for daily ongoing); **no NSE dependency** — India VIX is not used |
| D-007  | Collect Sensex index data daily (via AlphaVantage / BSE Bhavcopy) as the relative strength benchmark for feature engineering and as the input for volatility regime computation (D-006) |
...
| X-005  | Trades are executed as **limit orders** at the last-traded-price ± a configurable buffer (default: **0.3%**); buy orders use LTP + buffer, sell orders use LTP − buffer; **exception:** stop-loss exits (G-006 / IM-007) are submitted as **market orders** to guarantee execution — capital protection takes precedence over fill quality; new buy/sell proposals are generated and executed at **CP1 (9:30 AM)** only; stop-loss market sells (IM-007) may execute at any checkpoint (CP1, Mon-1 through Mon-5 IST); the 9:30 AM checkpoint is the primary execution window for the evening batch baseline plan; **unfilled limit order lifecycle:** limit orders submitted at CP1 that remain unfilled are **auto-cancelled** at the next checkpoint (Mon-1 onward) per IM-006a; no re-submission occurs at Mon-1 through Mon-5 — new proposals are only generated at CP1 |
...
Six structured trading decision windows run every trading day. They operate in two tiers:

- **Checkpoint 1 (9:30 AM — CP1) — Full Decision Checkpoint:** fetches live prices, runs LightGBM inference, re-ranks all watchlist stocks, generates buy and ranking-based sell proposals, applies all guardrails, and executes orders (or sends HITL in Trial mode).
- **Mon-1 through Mon-5 (10:30 AM, 11:30 AM, 12:30 PM, 1:30 PM, 2:30 PM) — P&L Monitoring Checkpoints:** fetch live prices only; evaluate stop-loss exits, daily loss limit, total drawdown, and volatility regime change. No LightGBM inference, no re-ranking, no new buy proposals.

This two-tier design retains full mid-session risk protection while eliminating the noise and complexity of running ML inference on incomplete intraday bars.

> **Accepted trade-off:** stop-losses and risk limits are checked at checkpoint times only. Events between checkpoints are not caught until the next check. Maximum unmonitored window is ~60 minutes. This is an intentional simplicity choice.

| ID     | Requirement |
|--------|-------------|
| IM-001 | Six intraday checkpoints run on every trading day: **9:30 AM (CP1), 10:30 AM (Mon-1), 11:30 AM (Mon-2), 12:30 PM (Mon-3), 1:30 PM (Mon-4), 2:30 PM (Mon-5)** IST. They operate in two tiers: **(a) CP1 (9:30 AM) is the Full Decision Checkpoint** — runs the complete cycle IM-003 through IM-013; **(b) Mon-1 through Mon-5 are P&L Monitoring Checkpoints** — run IM-003 (price fetch), **IM-006a (unfilled order cleanup)**, IM-007 (stop-loss), IM-008 (daily P&L), IM-009 (total drawdown), IM-010 (volatility regime), IM-011 (market-close cancellation), IM-012 (independence), IM-013 (logging) only; IM-004, IM-005, and IM-006 do not run at Mon-1 through Mon-5 |
| IM-002 | **Pre-market preparation (9:10 AM, before CP1):** compute Sensex 20-day annualized realized volatility from cached Sensex closes; apply G-007/G-008 volatility regime filters to the baseline plan from the previous evening's batch; this is not a trading checkpoint — no proposals are generated and no orders are placed |
| IM-003 | At each checkpoint, fetch one data item: the **partial intraday OHLCV bar** for relevant symbols (held positions + baseline plan proposals, typically 10-15 stocks) via the data pipeline — current session data; the **Sensex 20-day annualized realized volatility** for G-007/G-008 is computed from cached Sensex daily closes (not fetched per-checkpoint) |
...
| IM-006 | **(CP1 only) Trade execution — (a) Production / Sandbox:** execute approved proposals as **limit orders** (per X-005: LTP ± 0.3% buffer) via OpenAlgo; in Sandbox mode OpenAlgo operates in **mock/paper order mode** — orders processed by OpenAlgo but not forwarded to any real broker; fills simulated at fetched real-time price; unfilled limit orders from CP1 are auto-cancelled at the next checkpoint per IM-006a (no re-submission at Mon-1 through Mon-5); **(b) Trial mode:** send HITL proposals to Telegram with full trade details (stock, direction, quantity, current price, alpha score, guardrail status); human has **15 minutes** to approve/reject; any proposal not responded to within 15 minutes is **auto-cancelled** |
| IM-006a | **(Mon-1 through Mon-5) Unfilled order cleanup:** at each P&L monitoring checkpoint, query OpenAlgo for any unfilled or partially-filled limit orders submitted at CP1; **auto-cancel** all unfilled orders — the original LTP-based limit price is stale; log each cancellation with the reason (unfilled after ~60 minutes or more); partially-filled orders: the filled portion is retained; the unfilled remainder is cancelled; no re-submission occurs at Mon-1 through Mon-5 — new buy proposals are only generated at CP1 |
...
| IM-010 | At each checkpoint, compute the current **Sensex 20-day annualized realized volatility** from the most recent 20 trading days of Sensex closing prices (cached in Qlib dataset, updated daily by evening batch); check whether volatility has crossed a new G-007/G-008 regime threshold since the previous checkpoint; if a new threshold has been breached, apply the updated position sizing restrictions to all proposals at this checkpoint and send a Telegram alert; **volatility recovery (vol dropping below a threshold) does not restore position sizes during the same trading day** — the next evening's batch generates a fresh full-size plan; the conservative halved plan stands for the current day |
| IM-011 | At **3:30 PM** (market close), cancel all remaining HITL-awaiting **buy/sell proposals (CP1 proposals only)** that have not received Telegram approval; stop-loss HITL requests are not cancelled here — they auto-execute via the 15-minute timeout defined in IM-007 and will already be resolved well before 3:30 PM; log all cancellations; the checkpoint process stops for the day |
...
| Time (IST) | Action |
|------------|--------|| 9:00 AM | **Broker session refresh (Phase 2 onwards):** auto-generate TOTP code from stored encrypted secret; authenticate with broker via OpenAlgo; obtain fresh API session token; retry up to 3× on failure; if all retries fail, halt trading for the day and send Telegram alert || 9:10 AM | Compute Sensex realized volatility from cached closes; apply G-007/G-008 volatility regime filters to the baseline plan from the previous evening's batch; **no orders placed** — this is preparation only |

### 5.2 Intraday Trading Checkpoints (Trading Days Only)

Six independent decision windows per day operating in two tiers (see §4.9 IM-001): **CP1 (9:30 AM)** runs the full pipeline (inference + re-ranking + proposals + execution); **Mon-1 through Mon-5 (10:30 AM–2:30 PM)** run P&L monitoring only (price fetch, stop-loss, drawdown, volatility regime).

| Time (IST) | Checkpoint | Summary |
|------------|------------|---------|
| 9:30 AM | **CP1 — Full Decision** | Fetch live prices → LightGBM inference → Re-rank → Generate proposals → Apply all guardrails → Execute or HITL (Trial) |
| 10:30 AM | **Mon-1 — P&L Monitor** | Cancel unfilled CP1 limit orders (IM-006a) → Fetch live prices → Stop-loss check → Daily P&L check (G-004) → Total drawdown check (G-005) → Volatility regime check (G-007/G-008) → Execute stop-loss sells only |
| 11:30 AM | **Mon-2 — P&L Monitor** | Same as Mon-1 |
| 12:30 PM | **Mon-3 — P&L Monitor** | Same as Mon-1 |
| 1:30 PM  | **Mon-4 — P&L Monitor** | Same as Mon-1 |
| 2:30 PM  | **Mon-5 — P&L Monitor** | Same as Mon-1; any still-pending stop-loss HITL approvals at 3:30 PM are auto-executed |
...
| Time (IST) | Action |
|------------|--------|
| 4:00 PM | Data collection — fetch EOD OHLCV for all watchlist stocks, Sensex closing level (also used for volatility regime computation) |
...
### 6.3 Market Regime Filters (Sensex Realized Volatility)

| ID     | Guardrail |
|--------|-----------|
| G-007  | If **Sensex 20-day annualized realized volatility > 30%** (elevated market stress): reduce all LightGBM-suggested position sizes by **50%** |
| G-008  | If **Sensex 20-day annualized realized volatility > 45%** (extreme stress / crisis): halt all new buy orders; allow only sell/exit orders |
...
| ID     | Guardrail |
|--------|-----------|
| G-010  | Maximum number of **new buy/sell proposals** per day: **10** (Production default), **3** (Trial default — all generated at the 9:30 AM checkpoint), **unlimited** (Sandbox — for simulation completeness); configurable per mode; stop-loss sells triggered at Mon-1 through Mon-5 do not count toward this limit |
...
| ID     | Deliverable |
|--------|-------------|
| P1-001 | Data ingestion pipeline (BSE Bhavcopy + Indian Stock Market API + Sensex index + BSE trading calendar) |
...
| P1-005 | Risk manager with all guardrails enforced (including Sensex volatility regime filters) |
...
| P1-012 | Intraday Trading Checkpoints — six daily decision windows (9:30 AM / 10:30 AM / 11:30 AM / 12:30 PM / 1:30 PM / 2:30 PM) in two tiers: **CP1 (9:30 AM)** — LightGBM inference, signal refresh, proposal generation, full guardrail enforcement, and order execution via **OpenAlgo mock/paper mode**; **Mon-1 through Mon-5 (10:30 AM–2:30 PM)** — P&L monitoring only (stop-loss, drawdown, volatility regime checks); no real broker in either tier |
...
| Intraday **Mon-1 through Mon-5** (hourly 10:30–14:30) — price fetch + volatility computation + P&L rule checks only; no inference | ~0.1–0.2 GB |
...
| E-008  | If the intraday checkpoint process cannot fetch live prices at a checkpoint, skip that checkpoint entirely and log the failure: **(a) CP1** — proposal generation, inference, re-ranking, and all guard­rail checks are skipped for that window; **(b) Mon-1 through Mon-5** — stop-loss and drawdown monitoring are skipped for that window; in both cases the skipped monitoring is flagged in the daily report as an unmonitored window |
...
20. The six intraday checkpoints operate in two tiers: CP1 (9:30 AM) is the sole full-decision window (inference + re-ranking + proposals); Mon-1 through Mon-5 (10:30 AM, 11:30 AM, 12:30 PM, 1:30 PM, 2:30 PM) are lightweight P&L monitors (stop-loss, drawdown, volatility regime checks only). This design avoids running ML inference on structurally incomplete intraday bars while retaining full mid-session risk protection.
...
| Intraday Trading Checkpoints | Six intraday windows per trading day (9:30 AM CP1, 10:30 AM Mon-1, 11:30 AM Mon-2, 12:30 PM Mon-3, 1:30 PM Mon-4, 2:30 PM Mon-5) operating in two tiers: CP1 (9:30 AM) is the full decision window (inference, re-ranking, proposals, execution); Mon-1 through Mon-5 are P&L monitoring windows (stop-loss, drawdown, and volatility regime checks only) |
...
| Sensex Realized Volatility | Sensex 20-day annualized realized volatility computed from daily Sensex closing prices; used for market regime detection (G-007/G-008) replacing India VIX to eliminate NSE dependency |
...
*End of Requirements Document — v3.8*
